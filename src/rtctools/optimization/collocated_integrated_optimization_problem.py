...
class CollocatedIntegratedOptimizationProblem(OptimizationProblem, metaclass=ABCMeta):
    ...
    def transcribe(self):
        ...
        track_constraint_names = solver_opts.get("export_lp", False) or solver_opts.get(
    ...
    def _collint_constraint_names(self) -> list[str] | None:
        ...
        ``solver_options()``. Use ``track_constraint_names=True`` (without
        ``export_lp``) to access this list programmatically — e.g. for debugging
        or downstream processing — without writing an LP file.
    ...
    def solver_options(self):
        ...
        options.setdefault("track_constraint_names", False)
        return options
