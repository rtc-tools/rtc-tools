...
class CollocatedIntegratedOptimizationProblem(OptimizationProblem, metaclass=ABCMeta):
    ...
    def transcribe(self):
        ...
        function_options = {"max_num_dir": solver_opts["optimized_num_dir"]}
        ...
        if n_collocation_times > 1:
            if self.integrate_states:
                ...
                _map_options = self.map_options()
                if _map_options["mode"] == "thread":
                    accumulation = accumulated.map(
                        n_collocation_times - 1, _map_options["mode"], _map_options["n_threads"]
                    )
                else:
                    accumulation = accumulated.map(n_collocation_times - 1, _map_options["mode"])
        else:
            accumulation = None
        ...
        # Build per-row names only when requested: the list covers every collocation,
        # DAE, and delay row and can be large for complex models.
        export_opts = self.export_options()
        track_constraint_names = export_opts.get("export_lp", False) or export_opts.get(
            "track_constraint_names", False
        )
        ...
        for ensemble_member in range(self.ensemble_size):
            ...
            if constraints:
                ...
                if track_constraint_names:
                    base_names = _build_user_constraint_base_names(constraints, "constraint")
                    # Expand vector constraints (size1() > 1) to scalar base names. A scalar
                    # name shaped like an expanded name (e.g. "foo_0") could now collide with
                    # a vector constraint's expansion (e.g. "foo" of size 2); re-deduplicate
                    # after expansion to catch that.
                    expanded_names = []
                    for base, g_i in zip(base_names, g_constraint, strict=True):
                        s = g_i.size1()
                        if s > 1:
                            expanded_names.extend([f"{base}_{k}" for k in range(s)])
                        else:
                            expanded_names.append(base)
                    _deduplicate_constraint_names(expanded_names)
                    constraint_names.extend(f"{name}{member_suffix}" for name in expanded_names)
            ...
            if len(path_constraints) > 0:
                ...
                if track_constraint_names:
                    path_base_names_deduped = _build_user_constraint_base_names(
                    ...
                    _deduplicate_constraint_names(path_base_names)
        ...
        # optimize() also sets this from the returned discrete array, keeping both in sync.
    ...
    def _collint_constraint_names(self) -> list[str] | None:
        """Per-row constraint names, or ``None`` when name tracking is disabled.

        Each entry corresponds to a row in the constraint vector ``g`` returned
        by ``transcribe()``. The list is populated during ``transcribe()`` when
        either ``export_lp=True`` or ``track_constraint_names=True`` is set in
        :meth:`~rtctools.optimization.optimization_problem.OptimizationProblem.export_options`.
        Use ``track_constraint_names=True`` (without ``export_lp``) to access this list
        programmatically — e.g. for debugging or downstream processing — without writing
        an LP file.
    ...
    def _export_lp_file(
        ...
        constr_str, extra_bounds_str = _build_constraints(
