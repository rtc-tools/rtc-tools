...
class OptimizationProblem(DataStoreAccessor, metaclass=ABCMeta):
    ...
    def _export_lp_file(
        ...
        """
        if not hasattr(self, "_collint_variable_indices"):
            raise ValueError(
                "LP/MILP export is only supported for "
                "CollocatedIntegratedOptimizationProblem subclasses."
            )

        if not getattr(self, "linear_collocation", False):
            raise ValueError(
                "LP/MILP export requires linear_collocation=True. "
                "Set linear_collocation=True in compiler_options() to enable LP export."
            )
