...
class _ConstraintTuple(tuple):
    ...
    Instances are always returned by the goal-programming mixins from
    ``constraints()`` and ``path_constraints()``.
    """
...
class _GoalProgrammingMixinBase(OptimizationProblem, metaclass=ABCMeta):
    ...
    def _gp_build_constraint_list(self, goal_constraints) -> list:
        """Convert an iterable of _GoalConstraint objects to the public constraint list format."""
        return [
            _ConstraintTuple((c.function(self), c.min, c.max, _goal_constraint_name(c)))
