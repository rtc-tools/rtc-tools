...
def deduplicate_constraint_names(constraint_names: list[str]) -> list[str]:
...
def build_user_constraint_base_names(user_tuples: list, auto_prefix: str) -> list[str]:
    ...
    for i, c in enumerate(user_tuples):
        if len(c) > 3 and c[3]:
            ...
            validate_lp_constraint_name(name)
    ...
    deduplicate_constraint_names(base_names)
    return base_names


def validate_lp_constraint_name(name: str) -> None:
    ...
    handled separately in :func:`build_user_constraint_base_names`, which also renames
...
def build_objective(f: ca.SX, x: ca.SX, var_names: list[str]) -> str:
    ...
    if not objective_str:
        objective_str = "0"
    # Wrap at word boundaries only (spaces)
    wrapped_objective = "\n".join(
        textwrap.wrap(
            "  " + objective_str,
...
def build_constraints(
    ...
    # deduplicated names (e.g. via build_user_constraint_base_names). The sanitization
...
def build_bounds(
    ...
    Returns:
        Tuple of (bounds_str, binary_vars, general_vars) where bounds_str is the
        formatted bounds section (empty string when there are no bounds to emit),
        binary_vars is the list of binary variable names, and general_vars is the
        list of general integer variable names.
    """
...
def sanitize_var_names(indices_per_member: list[dict[str, list[int]]], num_total: int) -> list[str]:
    """
